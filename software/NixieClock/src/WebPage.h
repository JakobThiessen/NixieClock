...
<div class="fr">
<span class="fl">PROGMEM-Hintergrundbild</span>
<label class="tgl"><input type="checkbox" id="dp-bg"><span class="tgl-sl"></span></label>
</div>
<div class="ct" style="margin-top:10px">SD-Hintergrundbild (Kalender &amp; Wetter)</div>
<div style="font-size:.72rem;color:var(--mu);margin-bottom:8px">&#9888; Nur <b>BMP 24-bit</b> oder <b>JPG</b> &mdash; max. <b>160&times;128&nbsp;px</b> (gr&ouml;&szlig;ere Bilder werden geclippt), max. <b>100&nbsp;KB</b> &mdash; Datei muss in <code>/pict/</code> liegen</div>
<div class="fr" style="gap:6px;align-items:center">
<select id="bg-img" onchange="bgPreview(this.value)" style="flex:1;min-width:0;background:var(--bg);color:var(--tx);border:1px solid var(--bd);border-radius:6px;padding:5px 8px;font-size:.82rem"><option value="">&mdash; kein Bild &mdash;</option></select>
<button class="btn" onclick="loadBgImgList()" style="padding:5px 10px;font-size:.85rem;flex-shrink:0" title="Dateiliste von SD laden">&#8635;</button>
</div>
<div id="bg-pv" style="display:none;margin-top:6px;text-align:center"><img id="bg-pv-img" src="" style="max-width:100%;border-radius:6px;border:1px solid var(--bd)" alt="Vorschau"></div>
<div class="fr">
<span class="fl">Dunkle Schrift (f&uuml;r helles Bild)</span>
<label class="tgl"><input type="checkbox" id="bg-td"><span class="tgl-sl"></span></label>
</div>
</div>
<div class="card">
...
<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:.8rem;color:var(--mu)">
<span>&#128194;</span><span id="sd-cwd" style="color:var(--tx);flex:1">/</span>
<button class="btn" onclick="sdRescan()" style="padding:4px 12px;font-size:.8rem">&#8635; Aktualisieren</button>
</div>
<div id="sd-drop" style="border:2px dashed var(--bd);border-radius:8px;padding:6px 10px;margin-bottom:8px;font-size:.8rem;color:var(--mu);text-align:center;transition:.2s">
Datei per Drag &amp; Drop in den aktuellen Ordner ziehen &mdash; oder <label style="color:var(--ac);cursor:pointer"><input type="file" id="sd-pick" multiple style="display:none" onchange="sdPickUpload(this.files)">Datei ausw&auml;hlen</label>
<div id="sd-upbar" style="display:none;margin-top:4px;height:4px;background:var(--bd);border-radius:2px"><div id="sd-upprog" style="height:4px;background:var(--ac);border-radius:2px;width:0;transition:width .3s"></div></div>
</div>
<div style="display:flex;gap:12px;flex-wrap:wrap">
<div id="sd-tree" style="flex:1;min-width:200px;max-height:400px;overflow:auto;font-size:.85rem;border:1px solid var(--bd);border-radius:8px;padding:8px;background:var(--bg)"><div style="color:var(--mu);padding:20px 0;text-align:center">SD-Reiter &ouml;ffnen zum Laden</div></div>
...
<div style="color:var(--mu);padding:40px 0">Datei ausw&auml;hlen</div>
</div>
</div>
</div>
</div>
<div id="toast"></div>
<script>
var cMode=0,rgbInitDone=false,dispInitDone=false,clockInitDone=false,weatherInitDone=false,sdTabLoaded=false,curBgImg='';
function tab(id,b){document.querySelectorAll('.pn').forEach(p=>p.classList.remove('on'));document.querySelectorAll('.tb').forEach(x=>x.classList.remove('on'));document.getElementById(id).classList.add('on');b.classList.add('on');if(id==='sdc'&&!sdTabLoaded){sdTabLoaded=true;sdRescan();}}
...
if(!dispInitDone){var di=parseInt(gx(x,'DPINT'))||5;document.getElementById('dp-i').value=di;document.getElementById('dp-iv').textContent=di+' s';var db=parseInt(gx(x,'DPBR'))||4095;document.getElementById('dp-b').value=db;document.getElementById('dp-bv').textContent=Math.round(db/40.95)+'%';document.getElementById('dp-slide').checked=gx(x,'SLIDE')==='1';document.getElementById('dp-cal').checked=gx(x,'CAL')!=='0';document.getElementById('dp-bg').checked=gx(x,'BG')==='1';curBgImg=gx(x,'BGIMG');document.getElementById('bg-td').checked=gx(x,'BGTD')==='1';loadBgImgList();dispInitDone=true;}
...
function loadBgImgList(){fetch('/SD_LIST?path=/pict/').then(r=>r.json()).then(function(d){var sel=document.getElementById('bg-img');var prev=sel.value||curBgImg;sel.innerHTML='<option value="">&mdash; kein Bild &mdash;</option>';d.forEach(function(f){if(f.type!=='file'||f.size>102400)return;var n=f.name.toLowerCase();if(!/\.(bmp|jpg|jpeg)$/.test(n))return;var o=document.createElement('option');o.value=f.path;o.textContent=f.name+' ('+sdSize(f.size)+')';if(f.path===prev)o.selected=true;sel.appendChild(o);});bgPreview(document.getElementById('bg-img').value);}).catch(function(){});}
function bgPreview(p){var pv=document.getElementById('bg-pv');var im=document.getElementById('bg-pv-img');if(!p){pv.style.display='none';im.src='';return;}im.src='/SD_FILE?path='+encodeURIComponent(p)+'&t='+Date.now();pv.style.display='block';}
...
function saveDisp(){var i=document.getElementById('dp-i').value;var b=document.getElementById('dp-b').value;var sl=document.getElementById('dp-slide').checked?1:0;var ca=document.getElementById('dp-cal').checked?1:0;var bg=document.getElementById('dp-bg').checked?1:0;var bi=document.getElementById('bg-img').value;var bt=document.getElementById('bg-td').checked?1:0;fetch('/SET_DISPLAY?INTERVAL='+i+'&BRIGHT='+b+'&SLIDE='+sl+'&CAL='+ca+'&BG='+bg+'&BG_IMG='+encodeURIComponent(bi)+'&BG_TD='+bt).then(function(r){toast(r.ok?'Display gespeichert':'Fehler',r.ok);}).catch(function(){toast('Fehler',false);});}
...
var currentSdDir='/';
function sdRescan(){var tree=document.getElementById('sd-tree');tree.innerHTML='<div style="color:var(--mu);padding:20px 0;text-align:center"><div style="font-size:1.5rem;margin-bottom:6px">&#8635;</div>SD-Karte wird geladen...</div>';var btn=document.querySelector('button[onclick="sdRescan()"]');if(btn)btn.disabled=true;fetch('/SD_RESCAN').then(function(){setTimeout(function(){sdLoad(currentSdDir);},600);}).catch(function(){tree.innerHTML='<div style="color:var(--er)">Scan Fehler</div>';toast('Scan Fehler',false);}).finally(function(){if(btn)btn.disabled=false;});}
function sdLoad(p,_r){currentSdDir=p;document.getElementById('sd-cwd').textContent=p;fetch('/SD_LIST?path='+encodeURIComponent(p)).then(r=>r.json()).then(function(d){var h='';d.sort(function(a,b){if(a.type==='dir'&&b.type!=='dir')return -1;if(a.type!=='dir'&&b.type==='dir')return 1;return a.name.localeCompare(b.name);});if(p!=='/'){var pp=p.replace(/\/[^/]*\/?$/,'');if(!pp)pp='/';h+='<div style="cursor:pointer;padding:2px 0" onclick="sdLoad(\''+pp+'\')">&#128281; ..</div>';}d.forEach(function(e){if(e.type==='dir'){h+='<div style="cursor:pointer;padding:2px 0" onclick="sdLoad(\''+e.path+'\')">&#128193; '+e.name+'</div>';}else{h+='<div style="display:flex;align-items:center;justify-content:space-between;padding:2px 0"><span style="cursor:pointer;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" onclick="sdFile(\''+e.path+'\',\''+e.name+'\')">&#128196; '+e.name+' <small style="color:var(--mu)">('+sdSize(e.size)+')</small></span><button onclick="sdDelete(\''+e.path+'\')" style="background:none;border:none;color:var(--er);cursor:pointer;padding:0 4px;font-size:1rem;flex-shrink:0" title="L\xF6schen">&#128465;</button></div>';}});document.getElementById('sd-tree').innerHTML=h||'<div style="color:var(--mu)">Leer</div>';}).catch(function(){if(!_r){setTimeout(function(){sdLoad(p,true);},1500);}else{document.getElementById('sd-tree').innerHTML='<div style="color:var(--er)">Fehler beim Laden</div>';}});}
function sdFile(p,n){var pv=document.getElementById('sd-preview');var ext=n.toLowerCase().split('.').pop();if(['jpg','jpeg','png','bmp','gif'].indexOf(ext)>=0){var url='/SD_FILE?path='+encodeURIComponent(p);pv.innerHTML='<div style="margin-bottom:8px;font-weight:600">'+n+'</div>';var im=new Image();im.style='max-width:100%;max-height:300px;border-radius:6px';im.onerror=function(){var s=this;s.onerror=null;setTimeout(function(){s.src=url+'&r='+Date.now();},2500);};im.src=url+'&t='+Date.now();pv.appendChild(im);}else if(['json','txt','cfg','ini','log','csv'].indexOf(ext)>=0){pv.innerHTML='<div style="margin-bottom:8px;font-weight:600">'+n+'</div><div id="sd-txt" style="text-align:left;font-size:.75rem;color:var(--tx);background:var(--bg);border:1px solid var(--bd);border-radius:6px;padding:8px;max-height:340px;overflow:auto;white-space:pre-wrap;word-break:break-all">Lade...</div>';fetch('/SD_FILE?path='+encodeURIComponent(p)).then(function(r){return r.text();}).then(function(t){var el=document.getElementById('sd-txt');el.textContent=t;}).catch(function(){document.getElementById('sd-txt').textContent='Fehler beim Laden';});}else{pv.innerHTML='<div style="margin-bottom:8px;font-weight:600">'+n+'</div><div style="color:var(--mu);padding:20px">Keine Vorschau verf\xFCgbar</div>';}}
function sdDelete(p){if(!confirm('Datei l\xF6schen?\n'+p)) return;fetch('/SD_DELETE?path='+encodeURIComponent(p)).then(function(r){if(r.ok){toast('Gel\xF6scht',true);sdLoad(currentSdDir);}else toast('Fehler beim L\xF6schen',false);}).catch(function(){toast('Fehler',false);});}
function sdUploadFile(file){var fd=new FormData();fd.append('file',file,file.name);fd.append('dir',currentSdDir);var bar=document.getElementById('sd-upbar');var prog=document.getElementById('sd-upprog');bar.style.display='block';prog.style.width='0';var xhr=new XMLHttpRequest();xhr.upload.onprogress=function(e){if(e.lengthComputable)prog.style.width=Math.round(e.loaded/e.total*100)+'%';};xhr.onload=function(){bar.style.display='none';if(xhr.status===200){toast('Hochgeladen: '+file.name,true);sdLoad(currentSdDir);}else toast('Upload Fehler ('+xhr.status+')',false);};xhr.onerror=function(){bar.style.display='none';toast('Upload Fehler',false);};xhr.open('POST','/SD_UPLOAD?dir='+encodeURIComponent(currentSdDir));xhr.send(fd);}
function sdPickUpload(files){for(var i=0;i<files.length;i++)sdUploadFile(files[i]);}
(function(){var drop=document.getElementById('sd-drop');drop.addEventListener('dragover',function(e){e.preventDefault();drop.style.borderColor='var(--ac)';});drop.addEventListener('dragleave',function(){drop.style.borderColor='var(--bd)';});drop.addEventListener('drop',function(e){e.preventDefault();drop.style.borderColor='var(--bd)';var files=e.dataTransfer.files;for(var i=0;i<files.length;i++)sdUploadFile(files[i]);});})();
</script>
</body>
</html>
)=====";